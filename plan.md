...
### 検討したが採用しなかった代替案: システムのlibarchive

macOS標準の `/usr/lib/libarchive.2.dylib` は ZipCrypto / AES-256 の暗号化zip書き込みに
対応している（実機で生成し、標準unzipでの復号と誤パスワード拒否まで確認済み）。
採用すれば 6MB の同梱もLGPL義務も不要になる。それでも7zzを選んだ理由:

- `archive.h` が SDK に含まれておらず（`libarchive.tbd` はある）、Appleが公式に
  サポートするAPIではない。将来のmacOSで挙動が変わっても文句が言えない。
- 対応OSが macOS 13〜26 と幅広く、各バージョンのlibarchiveが同じ暗号化オプションを
  持つ保証がない。7zzはバージョンを固定して同梱するのでどのmacOSでも挙動が同じ。
- ディレクトリ再帰・進捗計算・エントリ書き込みを自前で実装する必要があり、
  コード量とバグの余地が大きく増える。

6MBと帰属表示のコストより、全対象OSで同じ挙動が保証されることを優先した。
...
**Google Drive で dmg を配り、受け取る人に初回だけシステム設定で許可してもらう。**
Apple Developer Program（$99/年）には加入しない。

実機検証で確認した事実:
- 隔離属性が付いた未署名アプリはmacOS 26でブロックされる
- ブラウザ経由のダウンロードでは必ず隔離属性が付く
- ただし**dmg自体は隔離属性が付いていてもマウントできる**ため、
  「dmgが開けない」で詰まることはない
- ad-hoc署名は有効（`valid on disk`）なので「壊れています」ではなく
  「開発元を確認できません」の扱いになり、システム設定から許可できる

配布物は dmg と `docs/インストール手順.md` の2点セット。
手順書なしで配ると受け取った人が警告で詰まる。

許可はアプリの版ごとに1回必要。更新のたびに再度発生するので、
更新が頻繁になるか配布人数が増えたら公証への切り替えを検討する
（`scripts/notarize.sh` は用意済み）。

### 残タスク

- [ ] アプリアイコンの追加
- [ ] 実際に1台のMacで配布フロー（DL→許可→起動）を通して手順書を検証
