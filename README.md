...
| 初回だけシステム設定で許可 | 無料 | 初回5分（版ごと） | 使える |

無料で済ませたい場合は、下2つのどちらかになります。
受け取る人向けの手順書は [docs/インストール手順.md](docs/インストール手順.md) にあります。

### Google Drive で配って手間もゼロにするなら公証（notarization）

**ブラウザでダウンロードした時点で隔離属性が付く**ため、署名なしのアプリは
ダウンロードした人の環境で起動できません。Apple Developer Program（年間99ドル）に
加入して公証を通せば、この制約が完全になくなります。
...
受け取る人の手順はダブルクリックとドラッグだけで、警告は出ません。
ただし配布経路を全員に守ってもらう必要があり、誰かがブラウザ経由で共有し直すと
そこで破綻します。

### 無料案B: Google Drive のまま、初回だけ許可してもらう

隔離属性が付いても、システム設定から「このまま開く」を選べば起動できます。
**版ごとに1回、1人5分程度**の手間が発生します。

配布時に [docs/インストール手順.md](docs/インストール手順.md) を一緒に渡してください。
なおこの手順書には「これ以外のアプリで同じ警告が出たら情報システム担当に相談する」
という注意書きを入れてあります。警告を無条件に許可する習慣を作らないためです。
